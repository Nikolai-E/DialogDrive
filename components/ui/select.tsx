...
const Select: React.FC<{
  ...
  return (
    <SelectContext.Provider value={{ open, setOpen, value, onValueChange, options, setOptions }}>
...
const SelectValue: React.FC<{ placeholder?: React.ReactNode }> = ({ placeholder }) => {
  const { value, options } = useSelect();
  const opt = options.find((o) => o.value === value);
  return (
    <span className="line-clamp-1 text-left flex-1 mr-2">{opt ? opt.label : placeholder}</span>
  );
};
...
const SelectItemAction = ({
  ...
  return (
    ...
    >
      <button
        type="button"
        className={cn('flex-1 flex items-center rounded-md py-0.5 pl-7 pr-2 text-left truncate')}
        onClick={() => {
          onValueChange?.(value);
          setOpen(false);
        }}
