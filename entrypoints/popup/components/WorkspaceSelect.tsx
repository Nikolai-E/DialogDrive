...
import { Select, SelectContent, SelectItemAction, SelectTrigger, SelectValue } from '../../../components/ui/select';
      ...
      <SelectContent>
        {workspaces.map((ws) => (
          <SelectItemAction
            ...
            onActionClick={ws === 'General' ? undefined : () => {
              if (confirm(`Delete workspace "${ws}" and move its items to General?`)) {
                if (value === ws) onChange('General');
                try { (useUnifiedStore.getState().deleteWorkspace as any)(ws); } catch {}
              }
            }}
          >
            {ws}
